...
resource "azurerm_virtual_machine_extension" "configure_ou_berlin" {
  name                 = "configure-ou-berlin"
  virtual_machine_id   = azurerm_windows_virtual_machine.vm_berlin.id
  publisher            = "Microsoft.Compute"
  type                 = "CustomScriptExtension"
  type_handler_version = "1.10"
  ...
  depends_on = [azurerm_virtual_machine_extension.bootstrap_ad_berlin]
}
